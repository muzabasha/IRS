...
import { BookOpen, Trophy, ArrowRight, Clock, Target, Sparkles, Activity, Star, LayoutDashboard, FileText } from "lucide-react"
...
export default async function Home({ searchParams }: { searchParams: Promise<{ [key: string]: string | string[] | undefined }> }) {
    ...
    const defaultTab = params.tab === "question-bank" ? "question-bank" : "overview"

    return (
        <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700 ease-out">
            <Tabs defaultValue={defaultTab} className="w-full space-y-8">
                <div className="flex items-center justify-between border-b pb-4">
                    <TabsList className="bg-muted/50 p-1 h-12 rounded-2xl border border-muted-foreground/10">
                        <TabsTrigger value="overview" className="rounded-xl px-6 data-[state=active]:bg-background data-[state=active]:shadow-sm gap-2">
                             <LayoutDashboard className="h-4 w-4" />
                             Overview
                        </TabsTrigger>
                        <TabsTrigger value="question-bank" className="rounded-xl px-6 data-[state=active]:bg-background data-[state=active]:shadow-sm gap-2">
                ...
                <TabsContent value="overview" className="space-y-8 mt-0 outline-hidden">
                    {/* Hero Section */}
                ...
                <TabsContent value="question-bank" className="mt-0 outline-hidden">
                    <QuestionBank />
                </TabsContent>
            </Tabs>
        </div>
